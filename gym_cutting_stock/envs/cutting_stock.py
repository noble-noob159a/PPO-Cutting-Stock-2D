...
class CuttingStockEnv(gym.Env):
    ...
    def _render_frame(self):
        ...
        canvas = pygame.Surface(new_size)
        canvas.fill((0, 0, 0))
        pix_square_size = 0.75  # The size of a single grid square in pixels
        ...
        for i, stock in enumerate(self._stocks):
            ...
            pygame.draw.rect(
                ...
                (128, 128, 128),
                pygame.Rect(
                    round((i % (window_size[0] // self.max_w) * self.max_w) * pix_square_size),
                    round((i // (window_size[0] // self.max_w) * self.max_h) * pix_square_size),
                    round(stock_width * pix_square_size),
                    round(stock_height * pix_square_size),
                ),
            )
